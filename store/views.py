...
class IndexView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['objects'] = Product.objects.select_related('category')
        return context
...
class ProductListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not self.request.GET.get('max_price'):
            filters_form = forms.FiltersForm(initial={
                'max_price': max_prod_price
            })
    ...
    def get_queryset(self):
        queryset = super(ProductListView, self).get_queryset().select_related('category')
        ...
        if form.is_valid() and 'min_price' in self.request.GET:
            if form.cleaned_data['categories']:
                queryset = queryset.filter(category__in=self.request.GET.getlist('categories'))

            queryset = queryset.filter(price__gte=self.request.GET['min_price'],
